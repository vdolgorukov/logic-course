...
* $|f(t_1, \dots, t_n)|_M^v = f^M(|t_1|_M^v, \dots, |t_n|_M^v) \in D$ для  $f \in Fn$, такая, что  $arity(f)=n$, $t_1, \dots, t_n \in Trm$.
...
* $M, v \models P(t_1, \dots, t_n)$ е.т.е. $\langle |t_1|_M^v, \dots, |t_n|_M^v \rangle \in P_M$, где $arity(P) = n$
* $M, v \models \neg \Phi$ е.т.е. $M, v \not \models \Phi$
